...
import { FlatList, TouchableWithoutFeedback, View, AsyncStorage } from 'react-native';
...
export default class Following extends Component {
    ...
    renderSeperator = () => { // Line Seperator between items in flatlist
        return(
            <View
                style={{
                    height:1,
    ...
    async getData(){ // use axios for Github API
        ...
        axios.get('http://api.github.com/users/'+username+'/following')
        .then((res) => {
            let tempData = res.data;
            let data = [];
            for (let i = 0; i < tempData.length; i++) { // push information of each following to data
            ...
            if (username == mastername) {
                AsyncStorage.setItem('@MySuperStore:following', JSON.stringify(data));
            } 
        })
        // Uncomment the following lines to test Storage

        // following = await AsyncStorage.getItem('@MySuperStore:following');
